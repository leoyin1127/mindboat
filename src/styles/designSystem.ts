...
export const designSystem = {
  ...
  patterns: {
    ...
    // Inner glow overlay for glass containers
    innerGlow: `
      absolute inset-0 rounded-3xl bg-gradient-to-br from-white/8 via-transparent to-transparent pointer-events-none
    `,

    // Tinted inner glow
    innerGlowTinted: `
      absolute inset-0 rounded-3xl bg-gradient-to-br from-slate-400/10 via-transparent to-transparent pointer-events-none
    `,
  }
};
...
export const getButtonStyle = (variant: 'glass' | 'glassTinted' | 'accent' | 'danger' = 'glass', size: 'xs' | 'sm' | 'md' | 'lg' = 'md') => {
  const baseStyle = designSystem.colors.buttons[variant]
  const sizeStyle = designSystem.spacing.button[size]
  const effects = `${designSystem.effects.transitions.default} ${designSystem.effects.hover.scale}`

  return `${baseStyle} ${sizeStyle} ${effects} ${designSystem.radius.md}`
}
